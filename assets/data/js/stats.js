...
function fetchRecordData(statsArray = [], prop = "weight") {
	const stats = statsArray.map((stat) => Number(stat[prop]));
	return stats;
}
...
/**
 * A collection of utility functions for working with exercise statistics.
 *
 * @namespace stats
 * @property {Function} getAllRecords - Retrieves all records for an exercise.
 * @property {Function} getLastTwoRecords - Retrieves the last two records for an exercise.
 * @property {Function} fetchRecordData - Extracts specific stats from record objects.
 * @property {Function} calculateAmountChange - Computes the difference between two values.
 * @property {Function} calcPercentageChange - Computes percentage change between two values.
 */
export const stats = {
	getAllRecords,
	getLastTwoRecords,
